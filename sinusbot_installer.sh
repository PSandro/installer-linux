...
if [[ $VIRTUALIZATION_TYPE == "openvz" ]]; then
	...
	redMessage "You're running the script inside a docker container. For that you need to set the time on the hypervisor directly."
else
	if [ -f /etc/centos-release ]; then
		service ntpd stop
...
# Don't use a startscript in a docker based installation
if [[ $VIRTUALIZATION_TYPE != "docker" ]]; then
	if [[ "$USE_SYSTEMD" == true ]]; then
